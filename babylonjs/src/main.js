...
scene.clearColor = new Color4(0.067, 0.075, 0.094, 1);

const camera = new FreeCamera("top-camera", new Vector3(480, 800, 270), scene);
camera.mode = FreeCamera.ORTHOGRAPHIC_CAMERA;
camera.orthoLeft = -480;
camera.orthoRight = 480;
camera.orthoTop = 270;
camera.orthoBottom = -270;
camera.upVector = new Vector3(0, 0, -1);
camera.setTarget(new Vector3(480, 0, 270));

new HemisphericLight("hemi", new Vector3(0, 1, 0), scene).intensity = 1.7;
...
const floor = MeshBuilder.CreateBox("floor", { width: ARENA.width, height: 4, depth: ARENA.height }, scene);
floor.position = new Vector3(ARENA.width / 2, -2, ARENA.height / 2);
...
for (const spec of [
  ...
  const border = MeshBuilder.CreateBox("border", { width: spec.w, height: 8, depth: spec.d }, scene);
  border.position = new Vector3(spec.x, 2, spec.z);
  border.material = mats.border;
}
...
for (const obstacle of OBSTACLES) {
  ...
  mesh.position = new Vector3(obstacle.x + obstacle.width / 2, 14, obstacle.y + obstacle.height / 2);
  mesh.material = mats.obstacle;
}
...
function resetGame() {
  ...
  for (const bullet of bullets) bullet.mesh.dispose();
  ...
  player = {
    ...
    mesh: MeshBuilder.CreateSphere("player", { diameter: PLAYER.radius * 2, segments: 24 }, scene),
  };
  player.mesh.position = new Vector3(player.x, PLAYER.radius, player.y);
  player.mesh.material = mats.player;
  aimLine = MeshBuilder.CreateCylinder("aim", { height: 30, diameter: 4 }, scene);
  aimLine.material = mats.aim;
  bullets = [];
  enemies = [];
...
function spawnEnemy(difficulty) {
  ...
  const mesh = MeshBuilder.CreateSphere("enemy", { diameter: ENEMY.radius * 2, segments: 20 }, scene);
  mesh.position = new Vector3(x, ENEMY.radius, y);
  mesh.material = mats.enemy;
  enemies.push({
    x,
    y,
    radius: ENEMY.radius,
...
function shoot() {
  ...
  const bullet = {
    ...
    mesh: MeshBuilder.CreateSphere("bullet", { diameter: BULLET.radius * 2, segments: 12 }, scene),
  };
  bullet.mesh.position = new Vector3(bullet.x, BULLET.radius, bullet.y);
  bullet.mesh.material = mats.bullet;
  bullets.push(bullet);
...
function updateGame(dt) {
  ...
  for (const enemy of enemies) {
    ...
    enemy.mesh.position.set(enemy.x, ENEMY.radius, enemy.y);
  }
  ...
  player.mesh.position.set(player.x, PLAYER.radius, player.y);
  updateAimLine();
...
function updateAimLine() {
  const aim = state.lastAim;
  const length = 30;
  aimLine.position = new Vector3(player.x + aim.x * length / 2, PLAYER.radius + 2, player.y + aim.y * length / 2);
  aimLine.scaling = new Vector3(1, 1, 1);
  aimLine.rotation = new Vector3(Math.PI / 2, 0, Math.atan2(aim.y, aim.x) + Math.PI / 2);
}
