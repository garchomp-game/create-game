...
import type { RulesetProfileId } from "../src/domain/ruleset";
...
export type ProbeResult = {
  ...
  eventHash: string;
  worldHash: string;
  exProtocolSelectedId: string | null;
  exProtocolSelectedAt: number | null;
  exProtocolExposureSeconds: number;
  exProtocolEvolutionOneReached: boolean;
  exProtocolEvolutionTwoReached: boolean;
  exProtocolMasteryReached: boolean;
  exProtocolLimitBreakReached: boolean;
};
...
export type ProbeRunOptions = {
  ...
  exProtocolPath?: ExProtocolProbePath;
};
...
export function registerV07ExpeditionProbeTests(): void {
...
export function executeExpedition(
  ...
  for (let frame = 0; frame < MAX_SECONDS * FRAME_RATE; frame += 1) {
    ...
    if (!options.inputTape && options.exProtocolPath) {
      input = createExProtocolProbeInput(
        session.world,
        session.config,
        input,
        options.exProtocolPath,
      );
    }
  ...
  const exProtocol = session.world.stats.exProtocolMetrics;
  const result: ProbeResult = {
    ...
    exProtocolSelectedId: exProtocol.selectedId,
    exProtocolSelectedAt: exProtocol.selectedAtElapsed,
    exProtocolExposureSeconds:
      exProtocol.counters.protocolExposureSeconds ?? 0,
    exProtocolEvolutionOneReached:
      exProtocol.evolutionOneAtElapsed !== null,
    exProtocolEvolutionTwoReached:
      exProtocol.evolutionTwoAtElapsed !== null,
    exProtocolMasteryReached:
      exProtocol.masteryAtElapsed !== null,
    exProtocolLimitBreakReached:
      exProtocol.limitBreakFirstAtElapsed !== null,
  };
