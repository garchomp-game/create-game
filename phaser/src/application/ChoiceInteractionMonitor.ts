...
export class ChoiceInteractionMonitor {
  ...
  private pendingResumes: PendingResume[] = [];
  ...
  observeResume(observation: ChoiceInteractionResumeObservation): void {
    ...
    const remaining: PendingResume[] = [];

    for (const pending of this.pendingResumes) {
      if (simulationSeconds <= pending.lastObservedAtSimulationSeconds) {
