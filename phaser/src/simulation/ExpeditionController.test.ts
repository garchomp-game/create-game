...
describe("ExpeditionController", () => {
  ...
  it("records explicit victory and defeat boundaries", () => {
    ...
    victory.world.state.elapsed = 420;
    ...
    expect(victoryEvents).toContainEqual(
      expect.objectContaining({ type: "expedition.completed", elapsed: 420 }),
    );
    expect(victoryEvents).toContainEqual(
      expect.objectContaining({ type: "game.over", elapsed: 420 }),
    );
  ...
  it("crosses all four Acts without a meaningful gap over 120 seconds", () => {
    ...
    for (let elapsed = 0; elapsed <= 420; elapsed += 0.25) {
      ...
      fixture.world.enemies.length = 0;
    ...
    ).toEqual(["deployment", "first-assault", "counterattack", "breakthrough"]);
    ...
    expect(events.some((event) => event.type === "expedition.completed")).toBe(true);
  });
});
