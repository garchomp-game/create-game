...
レビュー受領・採否済み。このページはWorkへ提出した時点の依頼文を再現する履歴スナップショットです。現在の決定はIssue #83、[v0.8 面白さの核の検証](../../design/core-promise-validation/)、[v0.8 実行計画](../../project-management/v08-execution-plan/)を優先します。

提出時のゲームルールbaselineはRC6 merge `0e07347424a8`、Expedition ruleset `phaser-v0.7.0-final-expedition-rc6`です。main HEADはCI追加後の`0ef2f4e50481`で、RC6以降にゲーム数値は変更していませんでした。production trafficはv0.6.8を維持し、選択画面の改善はDraft PR #84と別Previewで評価中です。
...
- 危険反転、技能ledger、選択テンポ、通常強化の候補偏り、武器教義、最大密度の実装順。
...
- `docs/src/content/docs/design/build-and-progression.md`
- `docs/src/content/docs/design/core-promise-validation.md`
- `docs/src/content/docs/design/current-pressure-curve-inventory.md`
- `docs/src/content/docs/design/hazard-reversal-preflight.md`
- `docs/src/content/docs/design/maximum-density-readability-preflight.md`
...
- `docs/src/content/docs/design/weapon-doctrine-preflight.md`
- `docs/src/content/docs/engineering/v08-observability-preflight.md`
...
- `docs/src/content/docs/project-management/v08-execution-plan.md`
...
リポジトリ全体を渡す場合も、`.git`、`node_modules`、`dist`、ローカルrun log、秘密情報は除外します。UI candidateは比較対象に含めますが、RC6ゲームルールと同じ採否へ混ぜません。
...
- 通常強化は最大済み候補を除外する独立重み付き3択で、EXのような一巡保証はありません。候補分布の検証は#92で武器教義と分けて扱います。
...
9. #83、#66、#76から#81、#92、#62、#64、#65の依存順、重複、不足、延期候補。
10. UI candidateの選択画面、最大密度の可読性、警告音について、ゲームルールと分けて測るべき点。
11. 通常強化の候補運について、現行維持、カテゴリ最低保証、通常bagのどれが、ビルド多様性と基礎操作の安定を両立するか。
