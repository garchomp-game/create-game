...
提出物再レビュー追補のcode commit `c908450a7101`では、自動ゲートを再取得済みです。65 files・420 passed / 2 skipped、normal / repair probe各`1 passed / 1 skipped`、Playwright 73 passed / 1 skipped、production buildを通過しています。最終HEAD `faeb3f5e8c5e`のVersion Previewで通常UI欠陥特化ランも完了し、RC6 controlをproduction候補として採用しました。
...
| `commander-terminal-objective` | Commander撃破、active timeout、deployment timeout | 完了。固有objectiveを消し、現在Actの通常objectiveへ戻る |
| `victory-pb-protection` | 勝利PB後に速い敗北を保存 | 完了。勝利PBは不変、敗北は履歴へ残る |
| `expedition-loss-history` | Expedition敗北を履歴表示 | 完了。`PB対象外: 遠征未完遂`を表示 |
| `centisecond-contract` | 0.295秒、PB差分、540秒メダル境界 | 完了。保存、比較、表示、差分、メダルが同じ整数centisecondを使う |
...
- Pulseの中央誘導・近距離周回をボス戦148.62秒、Spreadの通常戦術を155.36秒継続した。
- 勝利ランの回復相殺率は99.96%と98.21%だったが、位置、標的、回復経路を変えない安定循環にはならなかった。
- 不可視攻撃、操作不能、soft lock、記録損失、P0 / P1不具合は確認されなかった。
- 保存、PB scope、fixed seed、ruleset、敗北理由はunit / E2Eの記録契約と分けて保証した。

手動全勝ではなく、敗北1本を含む3本から難度と失敗理由を評価しました。詳細値と採否理由は[RC6 統合QAレポート](../v07-rc6-integration-report/)を正本とします。
...
5. RC6 controlで全回帰を完了し、同じVersion Previewで通常UIの欠陥特化ランを行う。
6. `PH-V07-008`でRC6 control採用を記録し、リポジトリ統合と配布候補の固定へ進む。
