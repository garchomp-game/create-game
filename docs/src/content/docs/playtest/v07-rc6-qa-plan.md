...
| ID | 条件 | 必須結果 |
| --- | --- | --- |
| `act-clock-block` | Commander active中にAct境界へ達する | Actは変わらず、解決後の次stepで1回だけ遷移 |
| `commander-spawn-start` | 予告後にspawn成功 | 120秒の起点がspawn eventと一致 |
| `commander-defer-retry` | 初回配置不可、後続で配置可能 | 同じ時刻、候補順、spawn IDで成功 |
| `commander-defer-timeout` | 期限まで配置不可 | `deployment-timeout`で終了しAct時計を解放 |
| `commander-active-timeout` | spawn後120秒未撃破 | `timeout`、撤退、重複終了なし |
| `victory-pb-protection` | 勝利PB後に速い敗北を保存 | 勝利PBは不変、敗北は履歴へ残る |
| `ranking-scope` | 2武器、複数勝利を保存 | overallとweapon PBが同時に正しい |
| `fixed-seed-partition` | 同rulesetで異なるfixed seed | seedごとのPBが混ざらない |
| `ruleset-partition` | RC5とRC6を保存 | RC6順位にRC5が混ざらない |
| `tactical-score-time-free` | 同戦術成果、異なる時間 | 戦術点が時間だけでは変化しない |

境界値として、spawn直前、期限と同時、119.999秒、120秒、Act境界と同一stepも含めます。

## 固定probe

3 fixed seed x 2武器の6構成を、同じシード集合と上限で実行します。RC6では仕組みの到達性ゲートとして**6構成すべての勝利**を要求します。

probeは次を集計ではなくassertします。

1. 全6構成が勝利する。
2. 全構成でCommanderが名目上のAct境界を越え、解決前にAct遷移しない。
3. 強制spawn defer fixtureの再試行結果がreplayで一致する。
4. 敗北保存後も勝利PBが保持される。
5. fixed seed実値ごとの順位が分離する。
6. overall PBと各weapon PBが両立する。

加えて、Commander、Act 5、boss phase 2、全攻撃種、勝利終了、event hash、world hash、保存hash、最大密度を既存RC5基準と比較します。6勝は人間向け難度の保証ではなく、自動入力で機構が詰まらないことの保証です。
