...
| T1 | 9課題Training後の同一gameplay | Trainingで学んだ分類と操作が通常戦へ転移するか測る | T2、#76、事前画像・動画、最小説明 |
...
cell: T0 | T1 | T2 | RC6 | #76-control | #76 | #93 | #94 | #95 | #92 | #79 | Integrated | UI
date:
facilitator:
buildCommit:
appVersion:
...
### T0 / T1初心者セッション: 事前教材なし

初心者をT0またはT1へ一度だけ割り当てます。T0はTrainingを見せず、T1はTrainingを完了してから同じPulseでEndlessへ進みます。Training自体の説明効果を測るため、通常の認識preflightより先に実施し、開始前には静止画、動画、最小説明、敵弾・XP・REPAIRの分類名を見せません。
...
3. 完了直後、同じPulseでEndlessを開始し、死亡または90秒まで無提示で観察する。
4. 自由回答と分類質問を記録する。
...
Rangedは60秒から始まるため、transfer probeは30秒で終了せず、次を別々に記録します。
...
対象cellの固定fixtureから1秒静止画と5秒clipを見せます。T0 / T1 / T2では初回の無提示runより前に実施せず、T1は総合演習とEndless 90秒probeの後に回します。
