...
[#97](https://github.com/garchomp-game/create-game/issues/97)の**T1前レビューで求められた必須修正、owner再確認、自動ゲート、固定Previewを完了し、Training実装のmain採用を決定**しました。初心者が説明なしで意味を理解し、通常戦へ知識を移せるかは未証明であり、[#81](https://github.com/garchomp-game/create-game/issues/81)で別に判定します。T1は現行の敵、敵弾、XP、REPAIR、強化UIを変えず、説明不足だけを切り分ける比較セルです。production traffic、Endless / Expeditionの数値、ruleset、RunRecord schemaは変更しません。

:::caution[2026-07-24 T1.1への更新]
...
| T1初版 | `2c0348133f02` |
| T1検証ゲート追補 | `f594206` |
| 接触課題実装 | `2a4fba8a890613be430c8ccf218a86c7522c2cb1` |
| 最終runtime候補 | `78b79da9c5aad7f7998ef7b5b1938f80850fdb6a`（最新main統合済み） |
| 配布build | `2247bd9cd16a` |
| Cloudflare Version | `7eaaf10f-fd82-4032-b363-5d4b44db8293` |
| 固定Preview | [v08-training-t1-contact-2247bd9](https://v08-training-t1-contact-2247bd9-arena-core.garchomp-game.workers.dev/) |
| 基点branch | `agent/v08-training-mvp` |
| mode / stage | `training` / `basic-training` |
| 武器 / seed | Pulse / `20260720` |
| 記録 | `recordPolicy: none` |
| 状態 | Training実装はmain採用。旧固定Previewは履歴扱いとし、T1.1の新Preview固定後に[#81](https://github.com/garchomp-game/create-game/issues/81)へ渡す |

## 実装した契約

- タイトルの主導線をEndless、Final Expedition、Trainingの3件、副導線を2列2行へ分けた。
- 移動、壁迂回、敵本体の接触ダメージ観察、指定敵撃破、XP回収、固定3択強化、敵弾2回回避、REPAIR、総合演習の9課題を決定論的に進める。接触観察後はHPを戻し、`撃破 -> XP -> LEVEL UP -> 強化`を一続きにしてから回避と修復へ移る。
...
- T1ではTraining前に静止画、動画、30秒説明を見せず、総合演習とPulse Endlessの死亡または90秒までのprobe後に分類質問と認識preflightを行う。
...
## 2026-07-23 接触ダメージ課題の追補

owner確認で、射撃練習では敵本体を倒す操作は学べる一方、敵本体との接触も被ダメージになることを明示していないと判断しました。射撃の直前へ観察課題を追加し、実装commitを`2a4fba8a890613be430c8ccf218a86c7522c2cb1`、最新main統合済みruntime候補を`78b79da9c5aad7f7998ef7b5b1938f80850fdb6a`へ更新します。

- 自機を`(200, 270)`、Chaserを`(760, 270)`へ置き、上下中央の開けたレーンで通常速度の接近を観察する。
- 観察中は移動、照準、射撃だけを無効化し、Pause、restart、タイトル復帰は維持する。
- 対象Chaserからの`player.damaged`かつ`source.kind: contact`を受けた場合だけ課題を完了し、時間経過だけでは進めない。
- 次のbriefingでは減ったHPを表示したまま「敵本体への接触でもHPが減る」と説明し、射撃練習を開始した時点でHPを100へ戻して同じ左右配置を再生成する。
- 旧runtime候補`e87250500304`とPreview `v08-training-t1-e872505`は履歴として残すが、初心者T1の実行対象には使わない。

この追補はTraining限定です。通常の接触ダメージ、敵速度、Endless / Expeditionの数値、ruleset、RunRecordを変更しません。

## 2026-07-23 owner再確認と統合判断

ownerが接触観察から射撃開始までを再確認し、敵が中央レーンを移動して接触すること、接触中に操作できないこと、減少HPを説明した後で射撃課題へ移ることを確認しました。操作導線として問題ないため、Training実装をmainへ採用します。

配布build `2247bd9cd16a`をCloudflare Version `7eaaf10f-fd82-4032-b363-5d4b44db8293`としてuploadし、固定Previewで版情報、通常run、Training開始、説明開始操作、console / page / request errorがないことを確認しました。これはproduction trafficへ配分していません。
...
| --- | --- |
| 型検査 | pass |
| unit / simulation | 69 files、468 passed / 2 skipped |
| Standard固定hash | event `0e5c664a`、world `47a80192`を維持 |
| production build | 207 modules、pass。既存の500KB chunk警告のみ |
| release smoke | Chrome landscape / portrait、Firefoxで基本出撃、公開情報、Training開始・1 step・退出の9件pass |
| Training公開入力E2E | 2件pass。pointer開始、WASD、接触中の入力ロック、接触警告、HP復元、mouse / Space、数字キーで全9課題を完了し、強化選択中のPause / 復帰も確認 |
| 非介入E2E | 非空の履歴 / PB / ランキングをfixture化し、完了、中断、restart、title、reload前後で記録、profile、settingsが完全一致 |
| visual | move / navigate / contact / aim / XP / REPAIR / transferを960 x 540と390 x 844で固定。接触と射撃は左右中央へ分離し、課題対象、4障害物、総合演習REPAIRをパネルが隠さない |
| 全Playwright | 最新main統合後の全89件を分割再取得し88 passed / 1 skipped。skipは明示opt-inの15分soak |
| Starlight build | 102 pages、pass |
| Version Preview smoke | Version `7eaaf10f-fd82-4032-b363-5d4b44db8293`、build `2247bd9cd16a`。版情報、通常run、Training開始を確認し、console / page / request / HTTP errorは0件 |
| GitHub Actions | evidence-sync head `2247bd9`の[run 29982936685](https://github.com/garchomp-game/create-game/actions/runs/29982936685)でPhaser quality、Starlight、Browser release smokeがpass。runtime treeは`78b79da`から不変 |
...
2026-07-23接触追補では、接触実装単体で対象unit 34件、全unit 450件、公開入力E2E 2件、Trainingと全visual 42件、全Playwright 85件、production buildを再取得しました。さらに最新main統合後、全unit 468件、全Playwright 89件を重複しないtest groupへ分割して88件pass / 1件skip、production build 207 modules、Starlight 102ページを再取得しました。接触観察、接触中の移動・射撃停止、警告中の減少HP、射撃開始時のHP復元を公開入力E2Eで分離してassertしています。
...
- 総合演習は固定時間で終了させず、初期敵3体の全滅と、演習中に生成された全Pickupの回収で即時完了する。継続的な生存確認は完了後のPulse Endlessを死亡または90秒まで観察するprobeが所有する。
...
2026-07-23の接触追補だけを戻す場合は`2a4fba8a890613be430c8ccf218a86c7522c2cb1`を単独で戻し、Training候補全体を不採用にする場合は`2c0348133f02`以降を戻すか、GameContentのTraining modeとタイトル導線を外します。通常モードは`recordPolicy: standard`を明示しており、保存済みRunRecordのmigrationやruleset変更は不要です。

後継runtime候補`78b79da9c5aa`はPR #99で最新mainとの競合解消、ローカルQA、PR CI、owner再確認、固定Previewを完了し、main採用を決定しました。#81ではbuild `2247bd9cd16a`の固定Previewを変更せず初心者観察へ進みます。誤認が解消した場合は[#98](https://github.com/garchomp-game/create-game/issues/98)のruntime視覚変更へ進みません。誤認が残った場合だけ、T2の単一visual candidateを別branch、別buildで比較します。
