...
[#97](https://github.com/garchomp-game/create-game/issues/97)の**実装候補は完成、採用判断は未実施**です。T1は現行の敵、敵弾、XP、REPAIR、強化UIを変えず、説明不足だけを切り分ける比較セルです。production traffic、Endless / Expeditionの数値、ruleset、RunRecord schemaは変更しません。
...
| 状態 | Draft。[#81](https://github.com/garchomp-game/create-game/issues/81)の人間T1待ち |
...
| --- | --- |
| 型検査 | pass |
| unit / simulation | 67 files、439 passed / 2 skipped |
| Standard固定hash | event `0e5c664a`、world `47a80192`を維持 |
| production build | 205 modules、pass。既存の500KB chunk警告のみ |
| release smoke | Chrome landscape / portrait、Firefoxの6件pass |
| Training公開入力E2E | pointer開始、WASD、mouse / Space、数字キーで全8課題を完了 |
| 非介入E2E | 完了、中断、restart、title、reload前後で履歴、ランキング、profile、settingsが完全一致 |
| visual | 960 x 540と390 x 844のtitle / Trainingを固定。HUD、指示、ボタンの重なりなし |
| 全Playwright | 77 passed / 1 skipped。skipは明示opt-inの15分soak |

初回全件実行で見つけたfocus順の旧期待値と、並列時の壁時計timeoutは修正し、同じ6ワーカー構成のfresh全件実行を通しました。
...
- 現行visual自体は変更していない。Training後も敵弾とPickupの誤認が残るかがT1の観測対象である。
- 外部telemetryと開発run exportを作らない。T1所感とraw countは#81へ手動記録する。

## rollbackと次の判断

不採用時は`2c0348133f02`を戻すか、GameContentのTraining modeとタイトル導線を外します。通常モードは`recordPolicy: standard`を明示しており、保存済みRunRecordのmigrationやruleset変更は不要です。

次はDraft PRのCIとT1 Previewを固定し、#81で人間観察を行います。誤認が解消した場合は[#98](https://github.com/garchomp-game/create-game/issues/98)のruntime視覚変更へ進みません。誤認が残った場合だけ、T2の単一visual candidateを別branch、別buildで比較します。
