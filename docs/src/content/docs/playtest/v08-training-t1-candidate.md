...
| T1初版 | `2c0348133f02` |
| T1検証ゲート追補 | `f594206` |
| 最終runtime候補 | `e872505003044b40e782c33203fb502c40057387`（2026-07-22 Work再レビューとowner確認を反映） |
| 基点branch | `agent/v08-training-mvp` |
| mode / stage | `training` / `basic-training` |
| 武器 / seed | Pulse / `20260720` |
| 記録 | `recordPolicy: none` |
| 状態 | runtime候補、ローカルQA、owner gate、PR CI、immutable Previewを固定済み。候補を変更せず、[#81](https://github.com/garchomp-game/create-game/issues/81)の初心者T1を待つ |
...
| Version Preview smoke | pass。Version ID `f3b88d64-c430-4985-8a55-f8c0e17f4a44`、alias `v08-training-t1-e872505`、`buildCommit: e87250500304`、console / page / request / HTTP error 0件 |
| GitHub Actions | [run 29906507015](https://github.com/garchomp-game/create-game/actions/runs/29906507015)でPhaser quality、Starlight build、Browser release smokeの3 jobがpass |
...
runtime候補`e87250500304`はDraft PR #99、CI、immutable Version Previewへ固定済みです。次は#81で初心者観察を行い、誤認が解消した場合は[#98](https://github.com/garchomp-game/create-game/issues/98)のruntime視覚変更へ進みません。誤認が残った場合だけ、T2の単一visual candidateを別branch、別buildで比較します。この証拠同期以後に文書だけを更新しても、T1で使うruntime SHAとPreviewは変更しません。
