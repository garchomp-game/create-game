...
| 状態 | 後継runtime候補とローカルQAを固定済み。owner再確認、PR CI、新immutable Previewの取得後に[#81](https://github.com/garchomp-game/create-game/issues/81)の初心者T1へ渡す |
...
| GitHub Actions | runtime候補`78b79da`では未取得。旧候補は[run 29906507015](https://github.com/garchomp-game/create-game/actions/runs/29906507015)で3 job pass |
...
後継runtime候補`78b79da9c5aa`はDraft PR #99へ置き、最新mainとの競合解消とローカルQAを完了しています。owner再確認、PR CI、新immutable Version Previewを同じSHAへ結び付けてから#81の初心者観察へ進みます。誤認が解消した場合は[#98](https://github.com/garchomp-game/create-game/issues/98)のruntime視覚変更へ進みません。誤認が残った場合だけ、T2の単一visual candidateを別branch、別buildで比較します。
