...
| T1初版 | `2c0348133f02` |
| T1検証ゲート追補 | `f594206` |
| 最終runtime候補 | `2a4fba8a890613be430c8ccf218a86c7522c2cb1`（敵本体の接触ダメージ観察を追加） |
...
owner確認で、射撃練習では敵本体を倒す操作は学べる一方、敵本体との接触も被ダメージになることを明示していないと判断しました。射撃の直前へ観察課題を追加し、T1候補を`2a4fba8a890613be430c8ccf218a86c7522c2cb1`へ更新します。
...
| 全Playwright | WebGL並列負荷を抑えた2 worker実行で85 passed / 1 skipped。skipは明示opt-inの15分soak |
| Starlight build | 101 pages、pass |
| Version Preview smoke | 後継候補`2a4fba8`では未取得。旧`e872505`の証拠はsuperseded |
| GitHub Actions | 後継候補`2a4fba8`では未取得。旧候補は[run 29906507015](https://github.com/garchomp-game/create-game/actions/runs/29906507015)で3 job pass |
...
2026-07-23接触追補では、対象unit 34件、全unit 450件、公開入力E2E 2件、Trainingと全visual 42件、全Playwright 85件、production buildを再取得しました。接触観察、接触中の移動・射撃停止、警告中の減少HP、射撃開始時のHP復元を公開入力E2Eで分離してassertしています。
...
後継runtime候補`2a4fba8a890`はDraft PR #99へ置き、ローカルQAを完了しています。owner再確認、PR CI、新immutable Version Previewを同じSHAへ結び付けてから#81の初心者観察へ進みます。誤認が解消した場合は[#98](https://github.com/garchomp-game/create-game/issues/98)のruntime視覚変更へ進みません。誤認が残った場合だけ、T2の単一visual candidateを別branch、別buildで比較します。
