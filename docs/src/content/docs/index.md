...
| --- | --- |
| 主実装 | Phaser版 |
| 現在の基盤 | productionはv0.6.8。採用済みv0.7 RC6はmainへ統合済み |
| 公開先 | [arena-core.garchomp-game.workers.dev](https://arena-core.garchomp-game.workers.dev/) |
| 次の作業 | 固定済みTraining T1候補を使った#81の人間転移確認。並行してEX Protocol local candidateの実GPU耐久と6体系の人間評価 |
| その次 | T1で誤認が残る場合だけ#98。UI Draft PR #84と#76 runtimeは別buildで評価 |
...
| 知りたいこと | 読むページ |
| --- | --- |
| 今何が動き、次に何をするか | [現在地](game/current-state/) / [直近フェーズ](project-management/next-phase-plan/) / [v0.8 実行計画](project-management/v08-execution-plan/) |
| どんなゲームを目指すか | [ゲーム方針](product/game-direction/) / [拡張設計の全体像](design/gameplay-expansion-blueprint/) |
| 外部助言とv0.8設計契約 | [外部ゲームデザイン助言メモ](design/external-game-design-advice/) / [Work提出スナップショット](playtest/v08-work-design-review-request/) / [v0.8 面白さの核の検証](design/core-promise-validation/) / [危険反転の実装前比較](design/hazard-reversal-preflight/) / [武器教義の実装前比較](design/weapon-doctrine-preflight/) / [EX Protocol候補](design/ex-protocols/) / [最大密度の可読性・警告音 事前監査](design/maximum-density-readability-preflight/) / [v0.8観測境界](engineering/v08-observability-preflight/) |
...
| 実際のプレイ結果と次回手順 | [手動プレイ記録](playtest/playtest-notes/) / [バランス回帰テスト](playtest/balance-probe/) / [RC6現行QA](playtest/v07-rc6-integration-report/) / [Training T1候補](playtest/v08-training-t1-candidate/) / [EX Protocol自動QA](playtest/v08-ex-protocol-candidate-report/) / [v0.8 構造化プレイテスト記録票](playtest/v08-structured-playtest-template/) |
