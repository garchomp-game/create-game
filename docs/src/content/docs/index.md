...
| --- | --- |
| 主実装 | Phaser版 |
| 現在の基盤 | productionはv0.6.8。採用済みv0.7 RC6を基準に、v0.8 control観測buildをDraft PR #113へ結合済み |
| 公開先 | [arena-core.garchomp-game.workers.dev](https://arena-core.garchomp-game.workers.dev/) |
| 次の作業 | #113のcontrol観測を9課題Training入りの最新mainへ修復統合し、同一buildでEndless / 最終遠征を両武器各1本と5分自由選択を観察する |
| その次 | Training T1で誤認が残る場合だけ#98へ進み、観測事実を基にCharger、選択UI、Boss、イベント緩和を1 build 1 candidateで比較する |
...
| 実際のプレイ結果と次回手順 | [手動プレイ記録](playtest/playtest-notes/) / [バランス回帰テスト](playtest/balance-probe/) / [RC6現行QA](playtest/v07-rc6-integration-report/) / [Training T1候補](playtest/v08-training-t1-candidate/) / [control観測build実施手順](playtest/v08-observation-control-runbook/) / [v0.8 構造化プレイテスト記録票](playtest/v08-structured-playtest-template/) |
