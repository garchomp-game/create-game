...
| 5 | PR #102 / #103 / #104 | Run Fact Kernel、最大密度fixture骨格、v0.8基盤状態をmain `f19889b`まで統合済み | 完了。各consumerは独立Issueで不足を追加 |
...
| T1 | [#97](https://github.com/garchomp-game/create-game/issues/97) | 現行visualのTrainingで説明不足を切り分ける | runtime候補、owner gate、CI、Previewを固定済み | #81でT0 / T1を分け、事前教材なしのEndlessを死亡または90秒まで観測 |
...
1. **完了**: 品質ゲート、Encounter境界、比較手順、v0.8正本をmainへ統合し、main `f19889b`まで基盤を更新した。
2. **完了**: #83の判断、#93から#95、#97 / #98の責務、#81の再利用レーンをStarlightへ同期した。
3. **完了**: #97のTraining runtime候補、CI、immutable Previewを固定した。残る採否は#81の初心者T1である。
