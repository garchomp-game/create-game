...
| 順序 | 対象 | 現在地 | 次のゲート |
| ---: | --- | --- | --- |
| 1 | [#86](https://github.com/garchomp-game/create-game/issues/86) / PR #87 | workflowはmainへ統合済み。公式障害は解消したが既存runは`startup_failure` | main SHAで3 jobのfresh greenを取得 |
...
2026-07-20のGitHub Actions障害は同日04:44 UTCに解消しました。障害中の`startup_failure`をコード失敗とは扱いませんが、過去runをgreenともみなしません。workflowを再実行し、対象SHAへjob-levelの成功を結び付けるまでrequired checkを迂回して統合しません。
...
Wave 0はsimulation非介入の契約と観測です。並行可能でも1 Issue・1 branch・1 Draft PRを守ります。#81は最後だけのQAではなく、baseline、単独candidate、統合buildで同じ手順を再利用する検証レーンです。
...
1. mainのCIがfresh greenで、#86の証拠がSHAへ結び付いている。
