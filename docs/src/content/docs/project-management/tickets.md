...
| `PH-V08-024` | [#95](https://github.com/garchomp-game/create-game/issues/95) | Standard / Assist / Practiceの記録契約 | P0 | division、modifier、PB分離、旧記録migrationを先に固定するWave 0F。OverloadはC2で廃止 |
| `PH-V08-025` | [#97](https://github.com/garchomp-game/create-game/issues/97) | 選択式・再プレイ可能な基本操作Training | P1 | 9課題runtime `78b79da9c5aa`、owner gate、PR CI、固定Previewを完了しmain採用。学習効果は#81の事前教材なし人間transferで判定 |
...
| `PH-V08-027` | 未作成 | EX Protocol 6体系candidate | P1 | isolated branchで機能、正本、全体回帰、release規模の自動gateを完了。実GPUと人間採否が残る |

`PH-V08-027`は外部Issueをまだ作成していません。PR化する場合は[#79](https://github.com/garchomp-game/create-game/issues/79)へ黙って上書きせず、scope拡張を別Issueとして作り、#79をpreflight / superseded / splitのどれにするか明記します。仕様は[EX Protocol候補](../../design/ex-protocols/)を正本とします。
