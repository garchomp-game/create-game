...
横断QAとして[#86](https://github.com/garchomp-game/create-game/issues/86)のGitHub Actionsをmainへ追加しました。型、unit、配布build、Starlight、短いrelease smokeをPR statusにし、probe、全画像、実GPU耐久、人間採否は変更内容に応じた手動ゲートとして残します。初回runは2026-07-20のGitHub Actions公式障害によりjob生成前の`startup_failure`となったため、設定を変更せず復旧後に再実行します。

RC6採用を止めなかった共通APIのLowリスクは[#88](https://github.com/garchomp-game/create-game/issues/88)で独立処理しました。debug相当の時刻ジャンプで期限切れspawnを出さず、fallback geometryが予告方向を増やさないことをfixture化し、Encounter数値と難度は変更していません。全unit、配布build、release smoke、normal probeはgreenで、GitHub Actionsの公式障害復旧後にPR statusを取得して閉じます。
...
| 順序 | Issue | 検証するもの |
| --- | --- | --- |
| 0 | [#83](https://github.com/garchomp-game/create-game/issues/83) | 緊張・緩和、near-miss、難易度支援、公平性の設計契約 |
| 1 | [#66](https://github.com/garchomp-game/create-game/issues/66) | 世界観、視覚言語、素材と音の境界 |
...
体験仮説と採否順は[v0.8 面白さの核の検証](../../design/core-promise-validation/)、外部助言は[外部ゲームデザイン助言メモ](../../design/external-game-design-advice/)、提出用プロンプトは[v0.8 Work再レビュー依頼](../../playtest/v08-work-design-review-request/)を正本とします。
...
RC6はこの条件を満たしてmainへ統合済みです。#83の設計レビュー、Draft PR #84のUI採否、#66以降のv0.8縦切りへ進みます。production trafficの切替は、採用するUIと配布SHAを固定してから別途行います。
