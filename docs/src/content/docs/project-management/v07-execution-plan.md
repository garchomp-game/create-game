...
状態: RC5の固定3 seed x 2武器は6勝、決定論replay、72件のE2E、production build、Version Preview smokeを通過しました。これはRC6の回帰基準です。#73から#75を実装し、6構成の明示assertと通常UI採否をやり直すまでproductionへ昇格しません。
...
- 重大 / 高の既知不具合がない。
- 自動6構成がすべて勝利し、CommanderのAct境界越えとspawn defer replayをassertする。
