---
title: v0.7 実行計画
description: 公開ベータの基準固定から最終遠征RC6のproduction採否までの依存順とチケット詳細。
---
...
v0.7のゴールは、既存Endlessを壊さず、構造化した戦闘展開、指揮個体、予兆付き敵、追跡ボスを接続した第10ステージ相当の高難度エクスペディションを1本完成させることです。

体験要件は[v0.7 最終遠征プロトタイプ](../../design/v07-first-expedition/)を正本とします。このページは実装順、技術境界、検証ゲートを定義します。
...
- `PH-ARCH-005`と`006`は完了しました。run config、seed、乱数列、world、記録、debug、AI、performanceを単一所有者へ移し、`ArenaScene`は653行の調停中心となりました。
- 公開基準は`experiment/auto-pilot-observer`へcommit・push済みです。v0.7実装はレビュー可能な別ブランチへ分け、既存の検証資料を破棄しません。
- Wave 2から5までを`agent/v07-first-expedition`へ独立コミットで統合し、指揮艦ボスと撃破勝利まで接続しました。RC2の手動所感を受け、現行ステージを第10最終遠征へ再分類しました。RC4でEndlessのC5到達性を改善し、RC5でボス周回・回復循環・長時間稼ぎを局所対策しました。RC5の自動ゲートとVersion Previewは完了しています。
- RC5は基準証跡として保持し、productionへ直接昇格しません。UI変更前のRC5基点`d16655a`からRC6 branchを分け、Encounter時計、Expedition記録scope、有限回復候補を順に実装します。
...
状態: 2026-07-17完了。Phaser非依存の定義、Act進行、決定論的バッグ、全ライフサイクル、指標を実装し、既存Endlessのイベント列を互換経路で維持しました。
...
状態: 2026-07-17完了。弧、挟撃、護衛編成、互換外周出現を決定論的に生成し、最低距離、障害物、活動領域、到達性、個体上限、予告時間を純粋fixtureで保証しました。
...
状態: 2026-07-17完了。増援指揮、方向予告、安全な護衛投入、支援速度補正、撃破時の即時解除、専用表示・音・Run指標を実装しました。
...
状態: 2026-07-17完了。方向固定の予告・準備・突進・硬直、障害物と外周の中断、画面内開始、同時2体上限、専用表示・音・攻撃別指標を実装しました。
...
### [PH-V07-006: エクスペディション縦切り](https://github.com/garchomp-game/create-game/issues/48)

状態: 2026-07-17完了。タイトルのモード選択、5 Act、6カード、構造化出現、Commander / Charger / 指揮艦ボスの固定登場、ボス撃破勝利、勝敗リザルト、mode / stage別記録、再挑戦を接続しました。旧`survive: 420`仮境界は削除済みです。
...
状態: 2026-07-17完了。静的な戦術表示背景、Commander / Chargerの輪郭強化、Act遷移と侵入方向の演出を追加し、desktop / portraitの画像基準と描画レイヤー別の計測を固定しました。ゲームルールとルール版は変更していません。
...
### [PH-V07-007: 指揮艦ボス戦](https://github.com/garchomp-game/create-game/issues/58)

状態: RC2の手動所感を受け、RC3で最終ボスへ再設計。HP 3400、障害物経路を使う追跡、固定照準の広域斉射、安全な挟撃増援、第2段階、通常ウェーブ継続、専用HP / 予告 / 音 / 撃破演出を接続しました。
...
### [PH-V07-008: RC6統合QAとproduction採否](https://github.com/garchomp-game/create-game/issues/59)

状態: 完了。RC5の固定3 seed x 2武器は6勝、決定論replay、72件のE2E、production build、Version Preview smokeを通過し、RC6の回帰基準にしました。#73から#75、Expedition難度時計、提出物再レビュー追補、全入力replay、65 files・420 passed / 2 skipped、normal / repair各`1 passed / 1 skipped`、73 passed / 1 skippedのE2E、production buildと配布検査を完了しています。通常UIではPulse 1敗1勝、Spread 1勝で両武器がphase 2と3攻撃種へ到達し、中央周回へ固定しないことを確認してRC6 controlを採用しました。production trafficは配布SHA固定までv0.6.8を維持します。

成果物:

- 単体、simulation、E2E、画像比較、production build、耐久確認。
- RC5の自動・手動証跡と、RC6の時計・記録・回復比較。
- RC6の3 fixed seed x 2武器probeと決定論replay。
- Pulse / Spread各1本以上の中央周回再試行。
- production昇格、再調整、棄却、v0.8へ進む判断。

完了条件:

- 重大 / 高の既知不具合がない。
- 自動6構成がCommander、Act 5、全攻撃種へ到達し、phase 2 fixture、両武器の自然phase 2到達、3/6以上かつ両武器の自然勝利がある。
- CommanderのAct境界越えは専用fixture、spawn deferと終了状態は全入力replayでassertする。
- 敗北による勝利PB上書き、fixed seed混在、overall / weapon PBの排他がない。
- 同一seed / input hash、保存移行、Endless回帰、WebGL非空を確認する。
- ボス中の実被ダメージ、実回復、相殺率、中央滞在、フレーム性能を記録する。

### [PH-V07-010: Encounter時計とCommanderライフサイクル](https://github.com/garchomp-game/create-game/issues/73)

状態: 完了。RC6 Wave 1。2秒間隔、10秒配置期限、spawn後120秒、timeout撤退を決定論fixtureで保証しました。

- `runElapsed`、`actElapsed`、`activeElapsed`の所有者を分ける。
- HUDはEncounter Directorの状態だけを表示する。
- Commander cardはAct時計を止め、120秒をspawn成功後から数える。
- spawn deferを決定論的に再試行し、配置期限で必ず解放する。

### [PH-V07-011: Expedition記録scopeとRC6 ruleset分離](https://github.com/garchomp-game/create-game/issues/74)

状態: 完了。RC6 Wave 2。勝利時間、戦術点、時間メダル、scope、seed、rulesetを保存fixtureとブラウザで保証しました。

- 勝利の総クリア時間を主記録にする。
- 速攻成果は時間メダルへ移し、戦術点から除外する。
- `overall | weapon`、random / fixed seed、RC5 / RC6を分離する。
- 敗北が勝利PBを上書きしない保存fixtureを追加する。

### [PH-V07-012: 最終決戦の有限回復予算を比較検証](https://github.com/garchomp-game/create-game/issues/75)

状態: 完了。RC6 Wave 3。2400 HP・補充なしのcandidate Aはcontrol 3/6勝利に対して0/6勝利となり棄却しました。既定controlを維持し、計測と切替可能な候補定義だけを残します。

- RC6基礎版をcontrolにし、有限repair budget候補をペア比較する。
- 回復相殺率を`ボス中の実HP回復量 / ボス中の総被ダメージ`へ統一する。
- 道中、Endless、武器数値を変えず、両武器の到達性と手動戦術を確認する。
- 詳細は[RC6 有限回復予算 比較結果](../../playtest/v07-rc6-repair-budget-report/)を参照する。

技術契約は[RC6の時計と記録規則](../../engineering/expedition-rc6-clock-and-ranking-adr/)、検証手順は[RC6 QA・採否計画](../../playtest/v07-rc6-qa-plan/)を正本とします。
...
RC5基点 d16655a -> PH-V07-010 -> PH-V07-011 -> PH-V07-012 -> PH-V07-008
...
| 4 | `PH-V07-006`、`PH-ARCH-007`完了 | ボス前までのExpeditionを開始からリザルトまで通し、視覚縦切りを評価する |
| 5 | `PH-V07-007` | 完了。両武器で可避なボスと勝利条件が成立 |
| 6 | `PH-V07-008` RC5証跡 | 完了。RC5自動回帰、性能、preview、文書を固定 |
| RC6-1 | `PH-V07-010` | Act境界、spawn起点、defer期限をfixtureで保証 |
| RC6-2 | `PH-V07-011` | 勝利PB、scope、seed、ruleset分離を保存fixtureで保証 |
| RC6-3 | `PH-V07-012` | control比較から有限回復候補を採用または棄却 |
| RC6-QA | `PH-V07-008` | 6構成probe、全回帰、手動欠陥特化ラン、production採否 |
...
- 危険反転event、技能shadow ledger、武器教義。
- 強化選択の停止時間と再開硬直の計測。
- 最大密度の視覚fixtureと警告音分離。
- 初心者・経験者の構造化プレイテスト。
- Stage 1 / 5 / 10はv0.9の3作戦検証へ送る。
- チャレンジ、解放条件、収集画面は3作戦の進行契約後へ延期する。
