---
title: v0.7 実行計画
description: 公開ベータの基準固定から最終遠征RC5の採否までの依存順とチケット詳細。
---
...
状態: RC3 / RC4の通常UI 5勝から、ボス周回中の回復が被ダメージの97.1%から99.5%を相殺し、長時間戦うほど得点が増える欠陥を確認しました。RC5で可避な制圧衝撃波、回復drop毎秒1個制限、完遂・速攻ボーナス、得点内訳リザルトを追加しました。固定3 seed x 2武器は6勝、決定論replay、72件のE2E、production build、Version Preview smokeを通過しています。production昇格は各武器1本以上の欠陥特化手動採否待ちです。詳細は[v0.7 統合QAレポート](../../playtest/v07-qa-report/)を参照してください。

成果物:

- 単体、simulation、E2E、画像比較、production build、耐久確認。
- RC3 / RC4のPulse 3本、Spread 2本の手動ランと、RC5各1本以上の欠陥特化確認。
- 採用、調整、削除、v0.8へ進む判断。

完了条件:

- 重大 / 高の既知不具合がない。
- 両武器でボス第2段階へ到達し、固定3シード内で各武器に少なくとも1勝がある。
...
| 6 | `PH-V07-008` | RC5自動回帰、性能、preview、文書を完了。production昇格は手動採否待ち |
