...
## 2026-07-24: EX Protocol C2は最新mainへ再統合し、人間ゲート前はDraftで保持する

Issue [#126](https://github.com/garchomp-game/create-game/issues/126)で、
`origin/main`の`60ae8889390c`を基点にEX Protocol C2を再統合する。
旧C2専用DOMは戻さず、採用済み`ArenaChoicePresenter`へProtocol、
Evolution、Limit Breakを接続する。9課題Training、desktop gate、
WebGL fallback、観測export、StudyLog契約を維持する。

通常buildはEX OFF、候補buildだけ`VITE_ARENA_EX_PROTOCOL_CANDIDATE=1`
でC2 profileを使う。過負荷契約は復活させず、新しいbalance scalarも変更しない。
自動の24 route、決定論、migration、probe、soak、browser、Training、
release smokeを通しても、Pulse / Spread各3体系の人間操作と実GPU確認までは
採用・production配分を行わない。

統合中に、共通PresenterのLimit Break表示分岐漏れとTraining E2Eの
固定入力量子化を検出した。前者は表示契約を回帰テストへ追加し、後者は
ゲーム本体を変えず距離依存のテスト入力へ修正する。

詳細は[EX Protocol C2 最新main統合レポート](../../playtest/v08-ex-c2-main-integration-report/)を参照する。
...
## 2026-07-24: Trainingの操作glyphをH0から提示する別候補を作る

決定: owner確認で、説明を1秒程度しか読まない初見ユーザーに対し、
5秒停止後に初めてWASDを出すH1候補では離脱防止が遅い可能性があると判断した。
既存H1 / H2候補を比較baselineとして残し、開始前とactive直後から
方向付きkey capを表示するH0即時ガイドを別candidateとして作る。

- 最初の移動は`W ↑ / A ← / S ↓ / D →`を位置関係どおり表示する。
- navigate、XP、REPAIRはlive位置とwaypointから必要なkeyを強調し、
  対象名と短い方向矢印を同時に表示する。
- contactは操作停止、aimはmouse、dodgeは上下回避、upgradeは数字keyを
  同じsemantic cueとしてbriefingへ表示する。
- H1は5秒無進展時の強調、H2は10秒無進展時の具体文と全経路guideにする。
- 総合演習は既習内容の転移を測るため、新しい操作promptを追加しない。
- cueのpure方向判定、Phaser描画、DOM描画を分離し、課題状態機械へ
  presentation都合を持ち込まない。
- 9課題、成功guard、retry、simulation、RNG、Profile、RunRecord、
  ruleset、production trafficは変更しない。

自動greenはcandidate完成の証拠であり、従来H1候補より初回入力が早くなることや
通常戦へ転移することの証明ではない。別参加者のfirst-input時間、方向誤操作、
H1 / H2到達、総合演習とEndless 30秒probeを比較して採否する。

## 2026-07-24: 難度固定・記録対象外のPractice Sandboxを独立モードにする

決定: 操作や敵役割を自由に観察できる練習場を`practice/practice-arena`として
Training、Endless、Assistから分離する。既定はHP無敵、出現量少なめ、
追跡体と重装体で、開始前にHP無敵、3段階の出現量、4種類の敵、武器を選べる。

- 1本の固定waveだけを使い、時間による速度、HP、出現上限の上昇を行わない。
- 危険イベント、後半契約、アリーナ崩壊、EX Protocolを無効にする。
- `recordPolicy: none`とし、履歴、PB、ランキング、報酬へ介入しない。
- 背景へ操作を常時薄く表示し、戦闘対象やHUDを覆う説明パネルは置かない。
- 設定はページ内の再開では保持するが、プロフィールへ永続化しない。

ボス練習、Pickup制御、敵速度の数値入力は初期候補へ含めず、利用所感を見て
個別に追加する。詳細は[練習場（Practice Sandbox）](../../design/practice-sandbox/)を参照する。

## 2026-07-25: 基本訓練をStory初期作戦へ包み、序盤成長を段階化する

決定: 製品タイトルの主導線を`ストーリー / エンドレス / 練習場`の3つへ絞る。
最終遠征はStory内の最終章へ移し、独立した基本訓練ボタンは公開画面から外す。

- Story第1章は`story/story-intro`、記録対象外の3任務10課題とする。
- 機体起動ではXPと強化を出さず、移動、接触、照準へ集中させる。
- 初回迎撃では連射強化Iだけを固定付与し、3体を倒すまで追加選択を出さない。
- 実戦投入前に初回の自由強化選択を置き、4体迎撃で効果を確認する。
- 旧9課題TrainingとStudyLog定義は比較・回帰用に内部保持する。
- Final Expedition、Endless、Practice、RunRecord、ランキング数値は変更しない。
...
追補: XP要求だけを表示すると、先に`3 / 3`へ到達したプレイヤーには
4個目のPickupがレベルアップ条件に見える。通常強化の待機中は
`経験値 3 / 3　強化までN秒`を表示し、XPバーと現在値を必要量で止める。
余剰XPの内部保持と8秒cadenceは変更しない。3個取得後、追加Pickupなしで
8秒境界に強化提示されることを結合fixtureで固定する。

## 2026-07-25: Endless序盤の敵役割を15秒ごとに分離する

決定: 初稿Candidate Bは30〜35秒spawn p50 11体、画面内敵平均が直前binの
2.94〜3.37倍となり、事前登録した停止条件で棄却した。Candidate B2として、
30秒は追跡体の密度だけを増やし、45秒で重装体、60秒で高速体、75秒で
射撃体を解禁する。

12 seed比較では30〜35秒spawn p50が両武器7体、画面内敵平均はPulse 1.61倍、
Spread 1.16倍だった。45秒境界もPulse 1.64倍、Spread 1.38倍で、
標準run生存p50は111.10秒 / 105.90秒となりCandidate Aから15%以内を維持した。

0秒band、90秒band、90秒以降の無制限成長、敵の共通cost / HP / damage、
武器、XPは変更しない。初回強化8秒cadenceと組み合わせた状態を、
初心者向けEndlessの次の人間評価候補とする。

wave開始4秒前の予告は設定の開始時刻と敵種差分から表示層で導出する。
30秒直後は進路確保を促し、spawn間隔を0.75秒から1.30秒へ落とす
...
「何をすると」「何が起きるか」の順に短くする。内部ID、保存schema、イベント名は
変更しない。

- Protocolは「固有スキル」、Evolutionは「強化」、Masteryは「完成能力」と表示する。
- Chargeは「準備」、Cooldownは「再使用まで」または「再装填」、
  RMBは「右クリック」と表示する。
- 交差導線と赤熱炉心の成立回数は、実判定と同じhelperから算出して表示する。
- 6能力に単色SVGアイコンを用意し、A / B比較のA案を22px固定で仮採用する。
- Pulse / Spreadの武器色は維持するが、色だけでなく輪郭でも能力を識別可能にする。
- アイコンは最終アートではなく、人間評価で再選択しやすさを測る識別候補とする。
...
## 2026-07-26: Endlessの危険イベントを5分台、アリーナ崩壊を15分へ分離する

棄却済み判断: Endlessの最初の危険イベントを300秒から330秒、以後の基準間隔を
90秒から110秒とする。通常の出現圧力は420秒、敵能力強化は600秒から維持し、
アリーナ崩壊だけを900秒開始へ移す。崩壊後の45秒ごと18px縮小、領域外damage、
3種の危険イベント内容は変更しない。

- 900秒まで危険イベントが見えない候補は、5分以降の展開変化が乏しいため採用しない。
- 崩壊は通常プレイヤーの必須到達点ではなく、熟練runと観戦AIが到達し得る最終圧力とする。
- 5分台の危険イベント、7分の出現圧力、10分の敵能力、15分の崩壊を別の時計にし、
  難度の上昇理由を一度に重ねない。
- Final Expedition、Story、Training、Practiceの時間軸は変更しない。
- 過去のC8 screeningとIntel GPU 15分ランは旧崩壊時間の履歴として保持し、
  新時間軸のバランス採否には使わない。
- productionへ採用する際はrulesetを分離し、同条件の旧ランキングと混在させない。

この判断は同日の手動確認で「全体的にもっさりする」と評価され、次の復元判断により
supersededしました。

## 2026-07-26: 旧テンポを段階導入wave付きで復元する

決定: XP 30開始と役割別の初期damage緩和は維持し、戦闘時間軸だけを従来へ戻す。

- 0秒は追跡体、30秒は追跡体の密度だけを小幅に上げる。
- 重装体45秒、高速体60秒、射撃体75秒、通常最終帯90秒と段階追加する。
- 危険イベントは135〜165秒、以後48〜68秒間隔へ戻す。
- 出現圧力は90秒、敵能力強化は240秒、アリーナ崩壊は600秒へ戻す。
- 崩壊は45秒ごと18px、イベント3種、画面密度上限を変更しない。
- Final Expedition、Story、Training、Practiceは変更しない。

理由: 5分まで危険イベントがなく、15分まで崩壊しない候補は、有限終了以前に通常区間の
反応と展開が遅く感じられた。一方、旧30秒waveをそのまま戻すと複数の難化要因が重なる。
30秒の追加を同種の密度だけに限定し、45〜75秒へ役割を分けることでテンポと可読性を
両立する。

## 2026-07-26: 初回XPを15、危険イベントを3分台へ調整する

決定: 段階導入wave、240秒の敵能力強化、600秒の崩壊は維持し、成長と最初の
危険イベントだけを中間調整する。

- 通常XPは15開始、1.05倍、上限60とする。
- 通常25強化の累計必要XPは1,081から705へ減らし、最後の必要量は48とする。
- 最初の危険イベントは180〜210秒へ移し、以後48〜68秒間隔は維持する。
- 敵wave、初期damage、回復、武器、EX、崩壊は変更しない。

理由: 初回30 XPでは戦闘テンポを戻しても最初の成長フィードバックが遅い。一方、
初回だけ15へ下げて後半を60のまま急増させると成長段差になる。1.05倍で全体を
約35%軽くし、最初の危険イベントを3分台へ離すことで、通常waveと強化を試す時間を
確保する。

## 2026-07-27: open Issueを単一Nextの課題解決キューへ再編する

決定: 初回導線と視覚品質をユーザーと一件ずつ判断するため、同時に
`status:next`を持つIssueを一件だけにする。親#135の下を次の順へ固定する。

1. #138 初回タイトルの主CTAとモード階層。
2. #139 Story初期作戦と最終遠征の階層・進行。
3. #140 プレイヤー向け用語と説明文。
4. #141 タイトル / Storyのグラフィカル縦切り。
5. #142 選択UI / HUD / リザルトの視覚統一。
6. #80 最大密度と警告優先度。
7. #81 初心者 / 経験者の統合人間検証。

現在は#138だけを`status:next`、後続を`status:queued`とする。Story拡張方針
#143は`status:deferred`とし、#81完了前に実装へ入らない。

同日に旧Issueを次の理由で閉じた。

- #76: control screeningでcharge 1/12、Pulse 0/6となり、候補成功条件が
  現行の強い行動と競合した。
- #77: Phase 0 fact kernelは完了し、利用者不在の汎用Presenterを先行しない。
- #79: 6体系のEX Protocol C2へ具体化され、別教義branchは重複する。
- #93: Boss Attack Cardとshadow観測は完了し、runtime欠陥は未確定である。
- #62 / #64 / #65: 現行Storyの初期作戦 + 最終遠征へ置き換わり、将来構造は
  中間作戦 / 難易度別 / ハイブリッドから再判断する。

Issueは削除せず、本文、コメント、比較結果を履歴として残す。v0.9 milestoneは
`Story Expansion Decision`へ変更し、旧Stage番号を製品要件として扱わない。

詳細は[課題解決キュー](../issue-resolution-queue/)を正本とする。
