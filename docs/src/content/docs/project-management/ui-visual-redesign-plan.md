...
残る課題は、同じ情報で比較可能な視覚草案と、世界観・素材・production UIライブラリの採否です。実際に見た目を変える前に比較環境で判断します。行数だけを目的にした分割、React / Vueの導入、全HUDのDOM化は行いません。
...
RC5の自動ゲートとpreview更新、`PH-V08-011`の表示境界は完了しました。次は`PH-V08-012`でproductionから分離した比較草案を作ります。productionへ反映する案とライブラリは、`PH-V08-010`の世界観判断と同じ情報・viewportでの比較が終わるまで確定しません。
