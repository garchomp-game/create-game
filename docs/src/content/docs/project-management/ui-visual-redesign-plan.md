...
- 最初は武器・通常強化・EX強化・契約の選択画面だけを採用案へ置き換える。
- `ArenaChoicePresenter`と入力callbackを維持し、Passive DOM Viewだけを変更する。
...
RC5の自動ゲートとpreview更新、`PH-V08-011`の表示境界、`PH-V08-012`の比較草案と採否は完了しました。次はRC5の人間プレイ採否と並行して、`PH-V08-010`で世界観・素材を絞り、ゲームルールへ触れない選択画面のproduction縦切りを実装します。
