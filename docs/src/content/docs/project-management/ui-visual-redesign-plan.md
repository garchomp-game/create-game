...
状態: RC6基点のDraft PR [#84](https://github.com/garchomp-game/create-game/pull/84)へ仮統合し、自動回帰完了。外部可読性確認と採否待ち。
...
RC6のゲームルール採否とmain統合、`PH-V08-011`の表示境界、`PH-V08-012`の比較草案、`PH-V08-013`の選択画面candidateまで完了しました。次はDraft PR #84の外部可読性確認と、#83のWork設計レビューです。採用前にproductionへmergeせず、選択後1秒の入力再開と被弾は#78の計測契約へ接続します。並行して`PH-V08-010`で世界観・素材を絞り、危険反転、技能成果、最大密度fixtureへA/B/Cの役割を展開します。Stage選択とStage 1 / 5のproduction実装はv0.9の3作戦検証へ送ります。
