...
1. Draft PR #99のTraining候補は表示遮蔽を含むT1前修正、owner gate、CI、immutable Previewを完了した。残る判断は#81の初心者T1である。
...
### 2026-07-21に確認したP1（解消済み）

当時の`PhaserTutorialLayer`の説明パネルは次の矩形をalpha `0.9`で覆っていました。
...
当時は、壁を迂回する課題、誘導地点、敵・XPの識別対象を説明UI自体が隠していました。visual E2Eも`move` stepだけで、後続stepの遮蔽を検出できませんでした。

人間T1へ渡す前の修正条件を次のように固定しました。
...
次はT1の開始blockerではありませんでしたが、runtime候補固定前にすべて解消しました。
...
| --- | --- | --- | --- | --- |
| [#97](https://github.com/garchomp-game/create-game/issues/97) | 任意・記録なしTraining | runtime候補とPreview固定済み、人間証拠待ち | 完了 | #81でadopt / revise / defer / reject |
...
| [#77](https://github.com/garchomp-game/create-game/issues/77) | candidate非依存fact kernel | Phase 0をmain統合済み | 完了 | 後続consumerで不足factを確認し、Presenter / summaryへ進む |
| [#94](https://github.com/garchomp-game/create-game/issues/94) | 主敗因、near-miss、同条件再挑戦 | Phase A do now | #77最小fact境界を統合済み | baseline cause match。未達時だけStandard候補 |
| [#80](https://github.com/garchomp-game/create-game/issues/80) | 共通capture harnessと最大密度ゲート | skeletonをmain統合済み | 完了 | candidate意味確定後に最大密度scenarioと警告優先度を追加 |
...
**状態: 完了。** PR #102をmain `10198a9e810b`へ統合しました。#77全体はPresenterと容量上限付きsummary永続化を残すためopenです。
...
**状態: 完了。** PR #103をmain `41ed5f04a9f4`へ統合しました。#80全体は最大密度fixture、警告優先度、視覚・音響採否を残すためopenです。
