...
| 基点 | `main` / `f19889b` |
| 結合候補 | [Draft PR #113](https://github.com/garchomp-game/create-game/pull/113) |
| 観測機能の結合commit | `4bd771e` |
...
| ラン終了 | 主な敗因、到達進捗、比較可能な差分 | 純粋なRun Outcome ViewModel |

Run Outcome ViewModelは将来のリザルト改善に使う事実層です。未登録の閾値で「あと少し」と煽る表示や、新しい助言UIは今回追加していません。
...
| ゲート | 結果 |
| --- | --- |
| Unit | 477 passed / 2 skipped |
| TypeScript | pass |
| Phaser production build | pass |
| Playwright E2E | 77 passed / 1 skipped |
| Starlight | 108 pages build pass |
| GitHub Actions | Phaser quality / Browser release smoke / Starlight buildの3件green |
| `main`との結合 | conflictなし |

E2Eのskipは明示的にopt-inとしている15分soakだけです。自動greenは計装と既存導線の回帰を保証しますが、面白さ、緊張と緩和、攻撃の学習性は保証しません。
