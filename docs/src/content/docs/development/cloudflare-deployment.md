...
v0.8 Training T1もproductionへ配分しません。総合演習の`9/9`表示と
戦闘object優先の描画順を反映したT1.1は、次の固定Previewで確認します。

| 項目 | 値 |
| --- | --- |
| Preview URL | `https://191dd49a-arena-core.garchomp-game.workers.dev` |
| Git commit | `59cffbf7cddf` |
| Cloudflare Version ID | `191dd49a-5b08-4edc-bfa1-0b7cc0cf0260` |
| 実URL確認日 | 2026-07-24 |

旧8課題版`v08-training-t1-e872505`と、固定`8/8`を含む9課題版
`v08-training-t1-contact-2247bd9`はsupersededです。履歴証拠として残しますが、
新しい初心者T1の母数へ混ぜません。

v0.8 EX Protocol C1は最新9課題Trainingを含む別candidateとして、productionへ配分せず次の固定Previewで確認します。

| 項目 | 値 |
| --- | --- |
| Preview URL | `https://v08-ex-protocols-c1-0380571-arena-core.garchomp-game.workers.dev` |
| アプリ / 既定ruleset | `0.8.0-candidate.1` / `phaser-v0.8-ex-protocols-c1` |
| Runtime commit | `03805713cf83` |
...
| Runtime commit | `f1bd1dd75758` |
| Cloudflare Version ID | `5c526482-f5d0-4b20-bed4-61335a188ef3` |
| 実URL確認日 | 2026-07-24 |

同じPreview aliasを新Versionへ付け替え、C2のProtocol数値を変えず、勝利時の不要な原因表示を除去し、敗北時の撃墜原因を平易な文へ更新しました。production trafficとC1 Previewは変更していません。

### C2 最新main統合Preview

2026-07-24に、採用済み選択UI、9課題Training、desktop gate、
WebGL fallback、StudyLog契約を含む最新mainへC2を再統合しました。

| 項目 | 値 |
| --- | --- |
| commit | `dca3cec1127b` |
| app / ruleset | `0.8.0-candidate.2` / `phaser-v0.8-ex-protocols-c2` |
| Version ID | `b6f88819-46c5-4820-8923-a2ff0c55e92c` |
| Preview URL | `https://v08-ex-c2-integration-dca3cec-arena-core.garchomp-game.workers.dev` |
| production traffic | 変更なし |

実URLで版情報、commit、canvas描画、Endless開始、ベータ情報、
第三者ライセンス、console / page / request error 0件を確認しました。
`wrangler versions deploy`は実行していません。

### v0.8 Training 即時ガイド統合Preview

2026-07-24に、C2統合候補へ即時WASD表示、固定mini panel、左右移動、
接触実演、静止・移動標的、REPAIR即時回収防止を統合しました。

| 項目 | 値 |
| --- | --- |
| commit | `6f6ac7259143` |
| app / ruleset | `0.8.0-candidate.2` / `phaser-v0.8-ex-protocols-c2` |
| Version ID | `6653cf94-e6a8-4472-ba26-d6ac265c4800` |
| Preview URL | `https://v08-training-guidance-6f6ac72-arena-core.garchomp-game.workers.dev` |
| production traffic | 変更なし |
