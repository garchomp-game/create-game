---
title: 操作設計
description: 移動、照準、自動射撃、防御行動、固有スキルの役割。
---

## 現在の操作

Arena Coreはキーボードとマウスを前提とするPC専用ゲームです。スマートフォン、タブレット、touch-only端末ではゲームを起動せず、PCで開き直す案内を表示します。画面幅だけでは判定しないため、狭いPCウィンドウとtouch対応PCは利用できます。

| 操作 | 入力 |
| --- | --- |
| 移動 | `WASD` または矢印キー |
| 照準 | マウス |
| 射撃 | 自動射撃、左クリック、`Space` |
| リスタート | `R` |
| 一時停止 | `Escape` または `P` |
| 操作ヘルプ | `H`、またはプレイ画面右下の `?` |
| デバッグ表示 | `F3` |

マウス照準が有効な間、Phaser入力アダプターで自動射撃する試作を導入済みです。

操作ヘルプはタイトルと設定画面の「操作ヘルプ」からも開けます。内容は「操作」「敵」「アイテム」の3タブに分け、プレイ中に開いた場合はシミュレーション時計を止めます。「アイテム」ではXPと、シアンのケースに白い十字を付けた「回復キット」を実ゲームと同じ見た目で案内します。閉じると、開く前のタイトル、設定、またはプレイ中のランへ戻ります。

初回の操作学習はタイトル上の独立した訓練ボタンではなく、
`ストーリー > 第1章 初期作戦`から開始します。左右移動、障害物迂回、
接触ダメージ、静止・移動標的、敵弾、回復キットを順に扱い、
固定の連射強化を体験した後に初めて自由な強化選択を行います。
詳細は[タイトル導線とストーリー初期作戦](../../engineering/story-onboarding-adr/)を参照してください。

productionでは右クリックと`E`に戦闘行動を割り当てていません。v0.8 EX Protocol candidateを明示的に有効にしたrunだけ、Active Protocolのspecialへ割り当てます。

| Candidate操作 | 入力 |
| --- | --- |
| Active Protocol | 右クリックまたは`E`の新規押下 |
| 右クリック時の照準 | pointer位置へ更新 |
| 同一frameの重複 | 右クリックと`E`を1回へcoalesce |

hold再発火、pause / choiceからのbuffer、右クリックによる通常射撃は行いません。候補OFFではEX用listener自体をinstallせず、旧操作を維持します。詳細は[EX Protocol候補](../ex-protocols/)を参照してください。

## 現在の課題

`WASD + マウス照準 + 射撃維持` は操作負荷が高く、慣れる前の早期死亡を増やします。

一方、照準と位置取りを自分で行うことは本作の上達要素です。自動射撃で不要な押し続け操作を減らしつつ、敵の優先順位と狙う方向は残します。

また、左クリックとSpaceキーの射撃機能は重複しています。ただしv0.5では記録、画面、設定を優先し、防御行動の追加はv0.6以降へ延期します。

## 自動射撃の現在仕様

- プレイ中にマウス照準が確立されたら自動で射撃する。
- タイトル、一時停止、強化選択、ゲームオーバー中のマウス移動は照準として保持しない。
- 左クリックとSpaceキーの射撃は互換用として残す。
- シミュレーション側の `InputSnapshot.shootHeld` は変更しない。
- メニューでは通常カーソル、ボタン上ではポインター、プレイ中だけ照準カーソルを使う。

## v0.5で採用する役割

| 入力 | 役割 | 状態 |
| --- | --- | --- |
| マウス | 照準 | 維持 |
| 自動射撃 | 通常攻撃 | 既定で有効。v0.5で設定化 |
| 左クリック | 手動射撃、互換操作 | 維持 |
| Space | 手動射撃、キーボード互換 | v0.5では維持 |
| 右クリック | productionでは割り当てなし | EX Protocol candidateだけspecialへ割り当て |
| Escape | 一時停止、戻る | 維持 |

自動射撃を無効にした場合も、左クリックまたはSpaceで射撃できます。設定要件は `PH-V05-010` で扱います。

## 後続で検証する防御ダッシュ

- 移動入力方向へ短距離移動する。
- 移動入力がない場合は照準方向を使う。
- 1回から2回の使用回数と明確な再使用表示を持つ。
- ごく短い無敵、接触ダメージだけ無効、無敵なしを比較する。
- 連打によって通常移動、障害物、敵接触の判断を無効化しない。

採用条件:

- 理不尽に感じる操作ミスを減らす。
- 常に通常移動より有利な移動手段にならない。
- プレイヤーが「使うべき瞬間」を説明できる。
- 使用回数、再使用可能時間、使用直後の被弾を記録できる。

## 検証方法

- 短時間、通常、高スコアの手動ランを比較する。
- 早期死亡が減るか確認する。
- 照準と位置取りによる上達余地が残るか確認する。
- 強化選択中やメニュー操作中に誤射しないか確認する。
- 操作方式をラン記録へ残せるようにする。

`balanceProbe` はシミュレーション入力の回帰検知であり、入力アダプター上の自動射撃が人間にとって快適かどうかは判定できません。最終判断には手動プレイを使います。

関連チケット:

- `PH-V04-001` マウス照準時の自動射撃試作。完了。
- `PH-V04-002` 防御ダッシュのキー配置試作。v0.6以降へ延期。
- `PH-V04-003` 右クリック固有スキルの入力分離。EX Protocol candidateでfeature限定実装。
- `PH-V04-004` Spaceキーの防御行動設計。後続へ延期。
- [`PH-V05-010`](https://github.com/garchomp-game/create-game/issues/10) 設定とアクセシビリティ境界。
