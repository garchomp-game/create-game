...
[#76 危険反転イベントの縦切り](https://github.com/garchomp-game/create-game/issues/76)で、最初から危険全体を作り直さず、Arena Coreらしい1件を比較可能な形へ絞ります。

このページは実装前の選定資料です。damage、半径、持続、得点、発生時刻、採用テーマは[#83](https://github.com/garchomp-game/create-game/issues/83)と[#66](https://github.com/garchomp-game/create-game/issues/66)の判断後に事前登録します。
...
| 結果 | プレイヤー被弾 | 妨害対象 | 意味 |
| --- | --- | ---: | --- |
| `avoided` | なし | 0体 | 基本回避に成功 |
| `reversed` | なし | 1体以上 | 回避と敵妨害を両立 |
| `traded` | あり | 1体以上 | 効果は出たが基本回避に失敗 |
| `hit` | あり | 0体 | 回避・反転とも失敗 |
| `expired` | なし | 0体 | timeoutで終了し反転機会なし |

`reversed`を成立させなくてもランを継続できるようにします。全Chargerで反転を必須にせず、障害物が近くにない進路は通常回避として扱います。

## 責務境界

| 責務 | 所有者 |
| --- | --- |
| 発生可否、半径、持続、対象種、同時上限 | content definition |
| Chargerの予告、突進、衝突理由 | 既存`chargerEnemySystem` |
| 衝突地点から対象IDを決定する純粋計算 | 新しいhazard reversal system |
| 敵の一時妨害状態と解除 | enemy simulation system |
| 結果eventと集約 | Simulation / stats system |
...
集約はattempts、outcome別件数、affected合計・最大、反転後の短い反撃区間だけを候補にします。詳細ID列は開発run exportへ残し、最初からローカル履歴へ無制限保存しません。
...
7. 同一seed / input / 30、60、120、144Hzでevent列と対象順が一致する。
8. Pulse / Spread双方で通常回避と反転成功が成立する。
9. candidate無効時にRC6のevent / world hashが一致する。
10. 最大密度とportraitで予告線、プレイヤー、敵弾、pulse ringが重なっても識別できる。
...
- 妨害半径、持続、減速率または完全停止。
- 対象となる通常敵と同時対象上限。
- 同じChargerが連続して反転を発生できる間隔。
- 反転後の観測時間窓。
- 人間テストで最初の反転機会を何秒以内に置くか。
- `reversed`を技能ledgerへ出す最低件数。

値は#83回答と固定マイクロfixtureを見てIssueへ事前登録します。実ラン結果を見てから有利なseedや閾値を選びません。
...
この第一候補は、#83と#66で「短い反撃窓」と警告語彙が採用された場合だけ実装へ進めます。
