...
この段階ではsimulation、乱数、RunRecord保存、PB、ランキング、結果画面を変更しません。入力は[#77](https://github.com/garchomp-game/create-game/issues/77)の`RunFactReadModel`と同じrunの`ObservedGameEvent`、出力は純粋な`RunOutcomeInsightViewModel`です。2026-07-24の#113修復で、純粋層をdebug snapshotとrun exportへ接続しました。
...
`RunLifecycleController`はstandard runの開始から終端まで、敗因と進捗に必要なeventだけを順序付きで保持します。通常敵への全hitは保持せず、Bossへのhit、被弾、撃破、Pickup、Commander、Boss、崩壊、run終端を対象にします。

- `recordPolicy: none`のTrainingはevent列とexportの対象外。
- debug snapshotとJSONでは`runOutcomeInsight`として参照する。
- run終了前は`not-reached / runNotTerminated`を返し、終端済みと混同しない。
- Workers観測Previewでは`downloadRunExport()`で人間がJSONを取得できる。
- 結果画面の文言、near-miss判定、助言UIはまだ変更しない。
...
- observation exportで実ラン事実を取得し、誤帰属や欠落がない。
