...
3. #97のT1を現行visualで固定し、#81でT0 / T1の事前教材なし90秒transferを観察する。誤認が残る場合だけ#98のT2へ進む。
4. #81でEndless / Expedition後の5分自由選択を取り、mode固有の再挑戦理由を確認する。
5. #78の選択wall-clock、#93のBoss / recovery shadow、#76のCharger control viabilityをsimulation非介入で分けて整える。
6. #94 Phase Aの事実集約とViewModelをruntime UIへ接続せず固定する。
7. Charger control viabilityを通過した場合だけ、#76の衝突妨害を別ruleset / SHAで試す。
8. 必要な場合だけ#93のBoss runtime候補を#76と別buildで比較する。
9. 記録軸の分離後に#94の結果・再挑戦UXを接続する。
10. #92通常強化と#79武器教義を別buildで個別評価する。
11. #80で採用済み要素の最大密度表現を固定し、#81で統合buildを確認する。
12. 人間検証を通った核だけをStage 1 / 5 / 10へ展開する。
...
- Practice / Assist / Overload gameplayの一括実装と、履歴依存の隠れた難度変更。
- 長編会話、カットシーン、ボイス。
