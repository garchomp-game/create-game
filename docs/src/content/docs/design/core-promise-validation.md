...
- Assist、PracticeをStandard PBへ混ぜない。
...
`Standard | Assist | Practice`を明示的なdivisionとして分けます。v0.8では3種類のgameplayを一括実装せず、division型、modifierの正規化、比較eligibility、旧記録migration、開始・結果表示の契約を先に保証します。
...
Overload divisionと240秒の契約選択はC2で新規runtimeから外しました。C1以前の保存記録は互換性のため読める状態を維持しますが、新しい比較区分や選択肢にはしません。
...
2026-07-23のhandoffを受け、Core完成後へ6体系の固定Protocol、E1 / E2、Masteryを挿入するcandidateを独立branchで実装しました。これはproduction採用ではなく、武器教義を操作可能な比較物へした段階です。

- 6体系 x E1 2択 x E2 2択の24 routeを自動保証する。
- Protocol offerは固定し、通常強化の候補運と混ぜない。
- 既存Limit BreakをLv3以降へ維持し、敵、XP曲線、通常武器数値を同時調整しない。
- effectが自然成立しないprobeを弱いと即断せず、機会不足と効果不足を分ける。
- 人間が選択後の行動変化、代償、武器弱点を説明できるまでproductionへ入れない。

詳細は[EX Protocol候補](../ex-protocols/)を参照してください。従来の「1武器1 branch」計画より広いため、採用時は分割比較またはscope変更を改めて判断します。
...
- Assist / Practiceのgameplay一括実装と、履歴依存の隠れた難度変更。
- 長編会話、カットシーン、ボイス。
