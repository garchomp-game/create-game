...
:::caution[設計履歴]
危険反転、技能ledger、武器教義、Boss runtimeの初期仮説を残す設計履歴です。
2026-07-27に旧候補を整理し、現在は入口・用語・視覚・最大密度・人間検証の
[課題解決キュー](../../project-management/issue-resolution-queue/)へ移行しました。
:::

:::note[2026-07-31の位置づけ]
[プロダクトコンセプト](../../product/game-concept/)で「包囲は、反撃の設計図になる」を
製品の北極星として採用しました。ただし、旧#76のCharger候補は成立せず終了しています。
北極星の採用と、特定candidateの成功を混同しません。

v0.8は入口、用語、視覚、最大密度、人間理解を整える現行キューを完了します。
具体的な包囲反転の実証と「11分目」のStory縦切りは、v0.8へ混ぜず後続の
単独candidateとして扱います。
:::
...
現行実装とこれまでのプレイ結果から、次をArena Coreの現在地として扱います。

> 四方から迫る圧力の中で、移動と照準を分離し、敵役割・障害物・回収経路を組み替えながら、倒し続けて生き延びる。

このコアは、Endlessでは上限のない生存と撃破点、Expeditionでは有限の戦況突破と総クリア時間へ接続されています。一方、「危険を反撃機会へ変える」「武器ごとに別の教義を完成させる」「敗北から次の一手を自動提示する」はまだ検証中です。

2026-07-22の批判的レビューに対する採否、停止条件、比較単位は[v0.8 批判的レビューの採用判断](../v08-critical-review-adoption/)を正本とします。

## v0.8の拡張仮説

**避け、狙い、戦況を読み替える技能が、次の瞬間と次のランの両方へ返ってくるゲーム**へ拡張できるか検証します。

この仮説を次の8本柱へ分けます。各柱は実装済みの特徴ではなく、単独candidateと人間観察を通過して初めて製品上の約束へ昇格します。
...
- simulationを変える支援はAssistとして明示し、mode、modifier、ruleset、記録eligibilityを記録する。
- Assist、Practice、Overloadを一つの強さ順へ並べず、Standard PBへ混ぜない。
...
- T1の初心者には事前のfixture、clip、30秒説明を見せない。Training、案内なし総合演習、同武器Endlessを死亡または90秒まで観察した後に分類質問を行う。
...
最初に現行Chargerが予告、突進、回復まで到達する機会をcontrolで確認します。control viabilityを通過した場合だけ、既存hazardを全面置換せず、敵にも作用する危険を1件だけ試します。

既存eventと副作用の比較、およびCharger衝突妨害の第一候補は[危険反転の実装前比較](../hazard-reversal-preflight/)へ整理しています。`spawned / killed-before-telegraph / telegraph / charge / obstacle / boundary / recovery`をcontrolで分離し、最初の経験者3名中2名が予告前撃破、または熟練runの大半でcharge未発生ならruntime candidateを停止します。control通過後にだけ、半径、持続、対象上限を#76へ事前登録します。
...
mode、simulation modifier、記録方針、比較eligibilityを別軸として扱います。既存の`division`識別子は互換用に維持できますが、Practice、Assist、Overloadを単一の強さ順として設計しません。v0.8ではgameplayを一括実装せず、modifierの正規化、比較eligibility、旧記録migration、開始・結果表示の契約を先に保証します。

- Standard policyは固定ルールで、従来のStandard PB対象。
- Assist modifierは同じ正規化条件内だけで比較する。
- Practice modeは比較記録と報酬を更新しない。
- Overload modifierは同じ正規化条件内だけで比較する。
...
通常強化は最大済み候補を除外し、重み付き抽選で3択を作ります。現状は最終的に通常候補を広く取得するため、大きなbuild identityではなく、取得順によるラン中盤までの判断差を主に作ります。一方、EXのような一巡保証はないため、連射や移動など基礎操作へ効く候補が長く出ないランを許容しています。
...
- 初心者: TrainingなしのT0とTrainingありのT1を参加者間で分け、事前教材なしのEndlessを死亡または90秒まで観察する。
...
- 必須run後に5分の自由選択を置き、同じmode、別mode、別weapon、終了のどれを自発的に選ぶか確認する。
...
2. 2026-07-22の採用判断を正本へ同期し、#77、#80の共通基盤を維持する。
3. 9課題の#97 Trainingを現行visualで固定し、#81でT0 / T1の事前教材なし90秒transferを観察する。誤認が残る場合だけ#98のT2へ進む。
4. #113のcontrol観測を最新mainへ再統合し、Endless / Expeditionの両武器runと5分自由選択からmode固有の再挑戦理由を確認する。
5. #78の選択wall-clock、#93のBoss / recovery shadow、#76のCharger control viabilityをsimulation非介入で分けて計測する。
6. #94 Phase Aの事実集約とViewModelをdebug exportへ接続し、production UIへの表示は別candidateに保つ。
7. #66の世界観と#70の選択UIはgameplay candidateと別buildで評価する。
8. Charger control viabilityを通過した場合だけ、#76の衝突妨害を別ruleset / SHAで試す。
9. 必要な場合だけ#93のBoss runtime候補を#76と別buildで比較する。
10. Standard / Assist / Practiceの記録軸分離後に#94の結果・再挑戦UXを接続する。
11. #92通常強化と#79武器教義を別buildで個別評価する。
12. #80で採用済み要素の最大密度表現を固定し、#81で統合buildを確認する。
13. 人間検証を通った核だけを、後続のStory構造決定と単独縦切りへ展開する。
...
- Practice / Assist gameplayの一括実装と、履歴依存の隠れた難度変更。
- 長編会話、カットシーン、ボイス。

これらは面白さの核と、一戦完結Storyの進行契約が固まってから再評価します。

## v0.8完了条件

- 読める危険、基本回避、反撃へ使える情報を統合候補で確認できる。
...
次のコンテンツ展開は
[課題解決キュー](../../project-management/issue-resolution-queue/)と
[ロードマップ](../../project-management/roadmap/)を正本とします。
[旧エクスペディション3作戦案](../expedition-campaign/)は再利用要件の履歴です。
