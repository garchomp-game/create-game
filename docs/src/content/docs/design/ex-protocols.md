...
:::caution[候補の状態]
このページは[#126](https://github.com/garchomp-game/create-game/issues/126)で最新mainへ再統合しているC2 candidate仕様です。production既定値はOFFで、production traffic配分と採用判断は行っていません。C1 Previewと自動QAは比較履歴として残し、C2はownerの可読性・成立頻度フィードバックを受けた別rulesetです。
:::
...
C2はC1の`opportunity=0`とowner feedbackを根拠に、発動条件と説明だけを改訂した比較候補です。最新main統合では対象micro fixture、全unit、型、短いprobe / soak、Final Expedition露出smoke、選択画面の横・縦browser fixture、9課題Trainingを通しました。結果は[最新main統合レポート](../../playtest/v08-ex-c2-main-integration-report/)と[C2 可読性・成立条件レポート](../../playtest/v08-ex-protocol-c2-readability-report/)へ分離します。C2の20 seed / 90秒release matrixと人間6体系確認は採用前ゲートとして残します。
