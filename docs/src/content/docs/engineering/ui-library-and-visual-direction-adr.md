...
採用。`PH-V08-012`の比較結果として、本番の基礎方向と最小の実装単位を決めます。世界観の固有名詞、最終bitmap素材、fontは`PH-V08-010`で引き続き検討します。
...
最初は**武器・通常強化・EX強化・契約の選択画面**だけを採用方向へ置き換えます。

1. `ArenaChoicePresenter`の表示モデルと選択callbackを維持する。
2. Aの情報階層と警告表現を基礎にする。
...
選択画面は`PH-V08-011`でPassive DOM Viewになっており、productionへの影響を最小化しながら高解像度文字と操作性を評価できます。タイトルとステージ選択は`PH-V08-007`、リザルトのDOM化はこの縦切りの評価後に別単位で行います。
