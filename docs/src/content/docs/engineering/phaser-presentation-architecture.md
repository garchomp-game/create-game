...
:::note[状態]
2026-07-17に`PH-ARCH-001`から`006`までを実装し、単体試験、production build、Playwright機能・画像回帰を通過しました。v0.7のStage / Encounter / Boss実装を受け入れられる所有権境界が成立しています。2026-07-18はRC5再監査で抽出した表示責務を`PH-V08-011`として分離し、既存画像を変えずに自動回帰を完了しました。
:::
...
行数だけでは分割しません。戦闘world描画は世界観と素材が決まる前に抽象化を増やさず、次は[UI・グラフィック再設計計画](../../project-management/ui-visual-redesign-plan/)のproduction外比較草案で変更対象を確定します。
