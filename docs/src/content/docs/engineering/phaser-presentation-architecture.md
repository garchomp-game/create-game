...
:::note[状態]
2026-07-17に`PH-ARCH-001`から`004`までを実装し、単体試験、production build、Playwright機能・画像回帰を通過しました。`PH-ARCH-005`以降は次の段階として残しています。
:::
...
| 対象 | 監査時 | 実装後 | 現在の責務 |
| --- | ---: | ---: | --- |
| `ArenaScene.ts` | 1274行 | 1129行 | Phaser lifecycle、ラン実行、debug、AI、各Controllerの調停 |
...
`ArenaScene`はまだ1000行を超えています。残りの主因はdebug API、fixture操作、ラン開始・終了、AI、性能計測であり、表示やメニューを再びSceneへ戻さず`PH-ARCH-005`と`006`で分けます。

実装時の回帰結果:

- `npm test`: 49ファイル、285件成功、2件skip。
- `npm run build`: TypeScript検査とproduction build成功。
- `npm run test:e2e`: 58件成功。15分耐久1件のみ通常実行ではskip。
- 固定画像比較: タイトル、設定、履歴、ランキング、プレイ、反射、リザルト、停止、選択画面を含め差分なし。
- `docs`のproduction build: 79ページ生成成功。
...
### PH-ARCH-005: SessionとRun Lifecycleの抽出（未着手）
