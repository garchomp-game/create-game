...
:::note[状態]
2026-07-17に`PH-ARCH-001`から`006`までを実装し、単体試験、production build、Playwright機能・画像回帰を通過しました。v0.7のStage / Encounter / Boss実装を受け入れられる所有権境界が成立しています。2026-07-18のRC5再監査では、この境界を保ったまま増えた表示責務を`PH-V08-011`へ整理しました。
:::
...
| 対象 | 監査時 | 実装後 | 現在の責務 |
| --- | ---: | ---: | --- |
| `ArenaScene.ts` | 1274行 | 629行 | Phaser lifecycle、入力収集、Controller間の調停、描画呼び出し |
...
| `AutoPilotController.ts` | なし | 109行 | AI有効状態、判断スナップショット、手動入力との合成 |
| `PerformanceMonitor.ts` | なし | 69行 | raw delta補正、集計、終了時性能スナップショット |
| `ArenaDebugController.ts` | なし | 525行 | debug API、fixture、soak protection、debug run export |
...
`PH-ARCH-005`でrun config、seed、乱数列、world、記録状態を単一所有者へ移し、`PH-ARCH-006`でdebug API、fixture、soak protection、AI状態、入力合成、性能集計を分離しました。`ArenaScene`は629行まで減り、残る処理はPhaser lifecycleとadapter間の調停が中心です。`ArenaDebugController`は開発専用APIという単一の変更理由を持つため、現時点では行数だけを理由に再分割しません。

実装時の回帰結果:

- `npm test`: 54ファイル、298件成功、2件skip。
...
v0.7でStage、Commander、Charger、3種のボス攻撃、Expedition得点、3列リザルトを追加した後の規模です。

| 対象 | 現在 | 判断 |
| --- | ---: | --- |
| `ArenaScene.ts` | 653行 | lifecycleとController調停を維持。新しい画面判断を戻さない |
| `ArenaSession.ts` | 215行 | Stage開始とworld進行の単一所有者として許容 |
| `PhaserArenaRenderer.ts` | 125行 | World、Screen、HUD、Feedbackの合成だけを維持 |
| `PhaserArenaWorldView.ts` | 863行 | entity / boss / attack描画が増加。世界観採用時に背景・boss FXを変更理由単位で抽出する |
| `PhaserArenaScreenView.ts` | 241行 | Passive Viewを維持 |
| `ArenaScreenPresenter.ts` | 450行 | ResultとSecondary Menuの表示モデルを実変更に合わせて抽出する |
| `ArenaChoiceOverlay.ts` | 398行 | データ整形とDOM接続、viewport同期を分ける |
| `PhaserHud.ts` | 388行 | Expedition / Boss HUDを含む。theme token導入時に値整形をPresenterへ寄せる |
| `ArenaMenuController.ts` | 336行 | 画面遷移と保存操作の所有者を維持 |

行数だけでは分割しません。次に独立した変更理由があるのは、画面別表示モデル、DOM選択のデータ整形、Phaser / CSSへ重複したtheme値です。[UI・グラフィック再設計計画](../../project-management/ui-visual-redesign-plan/)と[`PH-V08-011` #68](https://github.com/garchomp-game/create-game/issues/68)で扱います。戦闘world描画は世界観と素材が決まる前に抽象化を増やしません。

## RC6後のv0.8着手監査

2026-07-20に、#76から#80が触る変更面を基準に再確認しました。

| 対象 | main | UI candidate | 判断 |
| --- | ---: | ---: | --- |
| `ArenaScene.ts` | 677行 | 677行 | lifecycleと調停のまま。#78の集計式を置かず、Application monitorを接続するだけにする |
| `PhaserArenaWorldView.ts` | 866行 | 866行 | #66 / #80で戦況表現を変更する時にだけ、背景、Encounter / Boss予告、entity描画へ抽出する |
| `ArenaChoiceOverlay.ts` | 398行 | 239行 | candidateはDOM接続へ縮小し、346行の純粋`ArenaChoicePresenter`へ表示判断を移している |
| `ArenaScreenPresenter.ts` | 530行 | 530行 | #77の成果表示を直接積まず、ledger用Presenterを別の変更理由として追加する |
| `domain/types.ts` | 1407行 | 1407行 | 大半が宣言。行数だけでは分割せず、新しい一連のeventが独立した語彙になった時だけmodule化する |

1000行を超える`autoPilotTargeting.ts`、900行前後の観戦AI、1000行を超える試験ファイルも残ります。観戦AIは公開ルールから分離したopt-in実験で、試験は実行時責務を持ちません。v0.8本体の先行リファクタリング対象へ含めません。

実装前の判断は次で固定します。

1. #83回答前に大規模な移動や名前変更を行わない。
2. #70を採用する場合は、DOM Viewと純粋Presenterの境界を維持してmainへ統合する。
3. #78はwall-clockとsimulation時計をApplication層で集約し、SceneとDOMへ計算式を置かない。
4. #77はSimulationの決定論的eventを純粋集約し、`ArenaScreenPresenter`へ技能判定を埋め込まない。
5. #66 / #80で同じWorld Viewを複数の理由から変更する段階になったら、描画結果を変えない抽出commitを先に置く。

この条件なら、現在の大きなファイルを理由なく細分化せず、次の機能が実際に持つ変更理由に合わせて境界を作れます。#77 / #78の時計と保存境界は[観測可能性の事前監査](../v08-observability-preflight/)を参照してください。
...
### PH-ARCH-006: Debug / AI / Performanceの分離（完了）
...
Session、debug、AI、performanceの抽出まで完了しました。以降はStage / Encounterの製品変更をSceneへ積み増さず、applicationとsimulationの境界へ追加します。
