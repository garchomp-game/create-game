...
:::note[状態]
2026-07-17に`PH-ARCH-001`から`006`までを実装し、単体試験、production build、Playwright機能・画像回帰を通過しました。v0.7のStage / Encounter / Boss実装を受け入れられる所有権境界が成立しています。2026-07-18はRC5再監査で抽出した表示責務を`PH-V08-011`として分離し、`PH-V08-013`ではその境界を使って選択画面だけを採用UIへ変更しました。
:::
...
| 対象 | RC5監査時 | Wave 1後 | 現在の責務 |
| --- | ---: | ---: | --- |
| `ArenaScene.ts` | 653行 | 653行 | lifecycleとController調停を維持。新しい画面判断を戻さない |
| `ArenaSession.ts` | 215行 | 215行 | Stage開始とworld進行の単一所有者として許容 |
| `PhaserArenaRenderer.ts` | 125行 | 125行 | World、Screen、HUD、Feedbackの合成だけを維持 |
| `PhaserArenaWorldView.ts` | 863行 | 863行 | 世界観採用時に背景・boss FXを変更理由単位で抽出する |
| `PhaserArenaScreenView.ts` | 241行 | 242行 | 表示モデルを描くPassive Viewを維持 |
| `ArenaScreenPresenter.ts` | 450行 | 138行 | 画面種別とPresenter間の調停 |
| `ArenaResultPresenter.ts` | なし | 205行 | 終了要約、遠征得点、記録詳細 |
| `ArenaSecondaryMenuPresenter.ts` | なし | 110行 | 履歴、ランキング、設定とメニューラベル |
| `ArenaChoiceOverlay.ts` | 398行 | 188行 | DOM構築、focus、入力、viewport同期 |
| `ArenaChoicePresenter.ts` | なし | 292行 | 武器、通常 / EX強化、契約の表示モデル |
| `ArenaTheme.ts` / DOM変換 | なし | 70行 / 39行 | semantic tokenとadapter別形式への変換 |
| `PhaserHud.ts` | 388行 | 431行 | Expedition / Bossを含む戦闘HUD。共有tokenを利用し、派生ラベル分離はHUD再設計時に行う |
| `ArenaMenuController.ts` | 336行 | 336行 | 画面遷移と保存操作の所有者を維持 |

`ArenaScreenPresenter`と`ArenaChoiceOverlay`から独立した変更理由を抽出し、Phaser / CSSのUI値を`ArenaTheme`へ集約しました。新しいruntime依存はありません。逆依存を検出する境界試験を含む単体試験 `378 passed / 2 skipped`、型検査、production build、Playwright `72 passed / 1 skipped`と全画像比較を通過しています。

行数だけでは分割しません。production外比較でA「戦術管制」を基礎に選びましたが、戦闘world描画は世界観と素材が決まる前に抽象化を増やしません。以後も[UI・グラフィック再設計計画](../../project-management/ui-visual-redesign-plan/)の画面単位で変更します。

## 採用UIの最初の縦切り

`PH-V08-013`ではsimulationやControllerへ触れず、`ArenaChoicePresenter`の表示モデルと`ArenaChoiceOverlay`のPassive DOM Viewだけを拡張しました。

| 対象 | 現在 | 責務 |
| --- | ---: | --- |
| `ArenaChoicePresenter.ts` | 346行 | 画面文脈、候補番号、分類、ランク、説明、取得後数値、選択文 |
| `ArenaChoiceOverlay.ts` | 233行 | semantic DOM、focus、pointer、数字キー、Escape、viewport同期 |
| `arena.css` | 432行 | 選択画面だけのdesktop / compact / portrait表現 |

各ファイルは単一の変更理由を維持し、450行以下です。productionへTailwind、Lucide、別UI frameworkは追加していません。単体`378 passed / 2 skipped`、Playwright `73 passed / 1 skipped`、固定6ランのevent / world hash一致を通過しました。
