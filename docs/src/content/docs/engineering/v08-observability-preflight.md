...
[#95](https://github.com/garchomp-game/create-game/issues/95)は`division`、modifier、比較eligibility、migrationを所有します。#77はそのscopeを保持しますが、Standard / Assist / Practiceの比較規則を独自に決めません。C1以前のOverload記録は互換対象であり、新規divisionではありません。
