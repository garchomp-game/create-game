...
`ArenaScene`はラン開始、終了、画面間の調停を担当します。ゲーム世界、タイトル、リザルト、履歴、設定は`PhaserArenaRenderer`が描画し、武器・通常強化・EX・契約の選択は`ArenaChoiceOverlay`がDOMボタンとして描画します。DOM入力はフレーム単位の選択値へ変換して`InputSnapshot`へ合流し、シミュレーションからブラウザ要素を参照しません。v0.7で画面を増やす前に、残るCanvasメニューを専用Viewへ分ける余地があります。デバッグAPIは`ArenaDebugBridge`、固定状態はfixture群、ランJSON組み立ては`ArenaRunExport`へ分離済みです。
