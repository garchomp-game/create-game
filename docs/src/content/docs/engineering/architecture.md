...
`ArenaScene`はラン開始、終了、画面間の調停を担当します。ゲーム世界は`PhaserArenaWorldView`、タイトル、リザルト、履歴、設定は`PhaserArenaScreenView`、HUDは`PhaserHud`が描画し、51行の`PhaserArenaRenderer`が描画順を合成します。表示文とメニューラベルはPhaser非依存の`ArenaScreenPresenter`、メニュー状態と保存操作は`ArenaMenuController`が担当します。武器・通常強化・EX・契約の選択は`ArenaChoiceOverlay`がDOMボタンとして描画します。DOM入力はフレーム単位の選択値へ変換して`InputSnapshot`へ合流し、シミュレーションからブラウザ要素を参照しません。デバッグAPIは`ArenaDebugBridge`、固定状態はfixture群、ランJSON組み立ては`ArenaRunExport`へ分離済みです。

2026-07-17にPresentation Model、合成View、Menu Controllerへの第一段階を実装しました。`ArenaScene`は1274行から1129行、`PhaserArenaRenderer`は935行から51行になりました。Sceneに残るsession、run lifecycle、debug、AI、performanceの分離計画は[Phaserプレゼンテーション再設計](../phaser-presentation-architecture/)を参照してください。
...
観戦用オートプレイはゲーム本体の敵AIと分離し、戦術、照準、大域経路、局所回避を交換可能な境界にします。ライブラリ比較と予測回避の詳細は[観戦AIの設計判断](../auto-pilot-architecture/)を参照してください。

## プレイヤー弾の連続衝突

`bulletSystem.ts`は1フレームの弾道を、反射点で分割した移動線分として返します。障害物と固定外周の最初の交差、法線、反射後の残り時間はここで解きますが、敵へのダメージや撃破は扱いません。

`combatSystem.ts`は各線分と敵円の最初の交差時刻を求め、手前から`hitCapacity`まで命中を解決します。反射前後の区間へ`ricochetsUsed`を付けることで、集束共鳴の直接命中と反響回路の追撃を同じ位置情報から区別します。

この境界により、弾の移動・反射、戦闘結果、Phaserの描画を分離したまま、50msステップの最大Pulse弾速を固定試験できます。外周反射は`pulseBoundaryRicochet`で独立して無効化でき、敵移動、敵弾、出現、崩壊システムから参照しません。
