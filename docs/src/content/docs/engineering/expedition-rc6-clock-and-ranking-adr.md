...
採用。`PH-V07-010`の時計とCommanderライフサイクル、`PH-V07-011`の記録規則とRC6 rulesetは2026-07-19に実装済みです。`PH-V07-012`では2400 HPの有限回復候補を棄却し、RC6 controlを維持しました。統合QAでExpeditionの戦闘難度をAct進行へ同期し、production採否は通常UI確認待ちです。
...
通常の時間制Encounterは`blocksActClock: false`、Actをまたいで存在させない構造化Encounterは`true`を明示します。RC6ではこの宣言がAct時計と難度時計の両方を止めます。将来、Actだけを止めて難度を進めるEncounterが必要になった場合は別フラグを追加し、`blocksActClock`の意味を暗黙に変更しません。RC6のCommander cardは`true`です。選択時にAct時計を止め、完了または失敗時に再開します。これにより、Commanderが名目上のAct境界へ到達しても次Actは開始せず、解決後に決定論的に遷移します。
